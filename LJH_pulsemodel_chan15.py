...
for i in range(len(npulses)):
    ...
    amp=np.amax(tr)-ave
    tr=tr[515:1800]
    if amp>=1655 and amp<=1670:
        traces=traces+(tr-ave)
        count+=1
    elif abs(amp-3650)<=11.7:
...
np.save('pulsemodel_chan15.npy',pulsemod)
np.save('pulsemodel5400_chan15.npy',pulsemod1)
x=np.arange(1285)
plt.plot(x,pulsemod)
plt.show()
...
# y=plt.subplot()
# z=plt.subplot()
# plt.plot(x,(pulsemod/pulsemod1),label='amp 5400')
